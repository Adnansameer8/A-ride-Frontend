            ...
            {processedBookings.map(booking => {
              ...
              return (
                <tr key={booking.id} style={{ background: selectedBookings.includes(booking.id) ? '#fff7ed' : 'transparent' }}>
                  ...
                  <td>
                    <div style={{ fontWeight: '700' }} title={booking.customerName}>
                      {truncate(booking.customerName)}
                    </div>
                    <div style={{ fontSize: '12px', color: '#64748b' }} title={booking.email}>
                      {truncate(booking.email)}
                    </div>
                  </td>

                  <td style={{ fontWeight: '600', color: '#475569' }} title={booking.phone}>
                    {truncate(booking.phone)}
                  </td>

                  {/* <td title={booking.details?.tripName || booking.details?.trackName || booking.type}>
                    {truncate(booking.details?.tripName || booking.details?.trackName || booking.type)}
                  </td> */}
